...
class ClickpostController extends Controller
{
    ...
    public function downloadExcel(Request $request)
    {
        ...
        fputcsv($handle, array("ID", "Status", "Comments", "EmailNotification", "Shipping company", "Tracking number", "Cpid", "Reference Number", 'user_type','customer_name'));

        foreach($records as $record) {
            fputcsv($handle, array(
                $record['order_id'], 'Shipped', '','1', $record['shipping_company'], $record['awb'], $record['cp_id'], $record['reference_number'], $record['user_type'], $record['customer_name']
                )
            );
        }
